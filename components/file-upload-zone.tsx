...
export function FileUploadZone() {
    ...
    useEffect(() => {
        ...
        const dz = new Dropzone(dropzoneRef.current, {
            ...
            acceptedFiles: ".csv,.xlsx,.xls,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv",
        ...
        dz.on("error", (file, errorMessage) => {
            setFiles(prev => prev.map(f =>
                f.name === file.name ? {
                    ...f,
                    status: "error",
                    errorMessage: typeof errorMessage === "string" ? errorMessage : "Upload failed"
                } : f
            ));
        });
        ...
        <div className="w-full max-w-3xl mx-auto space-y-6">
            ...
            {files.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                    <h3 className="text-sm font-semibold text-gray-700 mb-4">Selected File</h3>
                        ...
                        <div key={index} className="space-y-3">
                            ...
                            {file.status === "success" && file.uploadResponse && (
                                <div className="space-y-3">
                                    ...
                                    {file.uploadResponse.summary && (
                                        <div className="grid grid-cols-4 gap-2 text-xs">
                                            <div className="bg-blue-50 px-3 py-2 rounded-lg text-center">
                                                <div className="font-semibold text-blue-900">{file.uploadResponse.summary.total}</div>
                                                <div className="text-blue-600">Total</div>
                                            </div>
                                            <div className="bg-green-50 px-3 py-2 rounded-lg text-center">
                                                <div className="font-semibold text-green-900">{file.uploadResponse.summary.inserted}</div>
                                                <div className="text-green-600">Inserted</div>
                                            </div>
                                            <div className="bg-yellow-50 px-3 py-2 rounded-lg text-center">
                                                <div className="font-semibold text-yellow-900">{file.uploadResponse.summary.duplicates}</div>
                                                <div className="text-yellow-600">Duplicates</div>
                                            </div>
                                            <div className="bg-red-50 px-3 py-2 rounded-lg text-center">
                                                <div className="font-semibold text-red-900">{file.uploadResponse.summary.errors}</div>
                                    ...
                                    {/* Duplicate Emails */}
                                    {file.uploadResponse.details?.duplicateEmails && file.uploadResponse.details.duplicateEmails.length > 0 && (
                                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                                            <p className="text-xs font-semibold text-yellow-900 mb-2">
                                                Duplicate Emails (skipped):
                                            </p>
                                            <div className="text-xs text-yellow-700 space-y-1 max-h-32 overflow-y-auto">
                                                {file.uploadResponse.details.duplicateEmails.map((email, idx) => (
                                    ...
                                    {/* Errors */}
                                    {file.uploadResponse.details?.errors && file.uploadResponse.details.errors.length > 0 && (
                                        <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                                            <p className="text-xs font-semibold text-red-900 mb-2">
                                                Import Errors:
                                            </p>
                                            <div className="text-xs text-red-700 space-y-1 max-h-32 overflow-y-auto">
                                                {file.uploadResponse.details.errors.map((error, idx) => (
