...
export default async function UserAnalyticsPage({ params }: PageProps) {
  ...
  // Fetch today's analytics from our API route (server-to-server call not needed; query directly)
  const now = new Date();
  ...
  // Resolve names for activity messages
  const { formatActivityLogMessage } = await import("@/lib/activity/audit-message");
  const contactIds = [...new Set(allLogs.map((r) => r.contact_id).filter(Boolean))] as string[];
  const orgIds = [...new Set(allLogs.map((r) => r.org_id).filter(Boolean))] as string[];
  ...
  // Compute stats
  let contactsAdded = 0;
  const orgsAffectedSet = new Set<string>();
  const contactsAffectedSet = new Set<string>();
  let fieldChanges = 0;
  ...
  const activities = allLogs.map((row) => {
    ...
    if (row.contact_id) contactsAffectedSet.add(row.contact_id);
    if (row.org_id) orgsAffectedSet.add(row.org_id);
  ...
  return (
    <UserAnalyticsClient
      ...
      stats={{
        ...
        contactsAdded,
        contactsAffected: contactsAffectedSet.size,
        orgsAffected: orgsAffectedSet.size,
        fieldChanges,
      }}
